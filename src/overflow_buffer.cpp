...
void OverflowBuffer::insert(KeyType key, ValueType value) {
    ...
    if (size_ >= kMaxCapacity) {
        // Corollary 3.1: leaf should have split before this; keep last-write wins
        // on the nearest slot rather than overflowing the flat buffer.
        if (pos < size_) pairs_[pos].second = value;
        return;
    }
